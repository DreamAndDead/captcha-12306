...
def train_image():
    ...
    with open('model/image-label.pkl', 'wb') as f:
        pickle.dump(l, f)
    ...
    # compose network
    model = build_lenet(width, height, depth, classes)
    ...
    # save the min val loss model
    checkpoint = ModelCheckpoint('model/image-classifier.hdf5', monitor="val_loss", mode="min",
                                 save_best_only=True, save_weights_only=False, verbose=1)
    callbacks = [checkpoint]
